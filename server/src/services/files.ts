                    ...
                    try {
                        ...
                        for (const feed of allFeeds) {
                            ...
                            while ((match = imageRegex.exec(feed.content)) !== null) {
                                ...
                                // 跳过外部链接
                                if (imageUrl.startsWith('http') && !imageUrl.includes(accessHost) && !imageUrl.includes(endpoint)) {
                                    continue;
                                }
                                extractedLinks.push({ feedId: feed.id, imageUrl, userId: feed.uid });
                            }
                        }
                        results.total = extractedLinks.length;
                        // 2. 按用户分组处理
                        const userGroups = extractedLinks.reduce((acc, item) => {
                        ...
                        for (const [userId, links] of Object.entries(userGroups)) {
                            for (const link of links) {
                                try {
                                    ...
                                    // 提取S3 key
                                    let key = link.imageUrl;
                                    if (key.startsWith(accessHost)) key = key.substring(accessHost.length);
                                    else if (key.startsWith(endpoint)) key = key.substring(endpoint.length);
                                    ...
                                    // 检查files表是否已存在该记录
                                    const existFile = await db.select({ id: files.id }).from(files).where(and(eq(files.path, key), eq(files.userId, Number(userId))));
                                    let fileId;
                                    if (existFile.length > 0) {
                                        fileId = existFile[0].id;
                                    } else {
                                        // 插入files表
                        ...
                        // 统计有图片的文章数量
                        const uniqueFeeds = new Set(extractedLinks.map(link => link.feedId));
