...
export function Pagination({
  ...
  const renderPreviousButton = () => {
    ...
    const baseClasses = "w-9 h-9 flex items-center justify-center rounded-full transition-all duration-300";
    ...
    <div className={`flex justify-center py-8 ${className}`}>
      <div className="flex items-center gap-2">
        ...
        {getPageNumbers().map((pageNumber) => {
          if (pageNumber === -1) {
            // 左省略号
            return <span key="ellipsis-left" className="text-gray-400 dark:text-gray-500 w-6 text-center flex items-center justify-center">
            ...
            // 右省略号
            return <span key="ellipsis-right" className="text-gray-400 dark:text-gray-500 w-6 text-center flex items-center justify-center">
              <span className="relative group">
                <span className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-theme/30 group-hover:w-full transition-all duration-300"></span>
                ···
              </span>
